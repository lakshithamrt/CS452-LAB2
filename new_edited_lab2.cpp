...
#include "shaders.h" //Inserting the header file for shader creation
...
theta=-pi/6;
	...
	for (int j=0;j<3;j++)
	{
	  vertexarray[j]= vec2 (R*cos(theta+2*j*pi/3),R*sin(theta+2*j*pi/3));// coordinates of one triangle for the star
	};
	...
	for (int j=3;j<6;j++)
	{
	  vertexarray[j]= vec2 (R*cos(theta+2*(j-3)*pi/3),R*sin(theta+2*(j-3)*pi/3));
	};	
...
//BuildShader(shaders);
  ...
  glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, 0, (void*)0);
   ...
   // Load shaders and use the resulting shader program
    GLuint program = BuildShader(shaders);
...
void drawscene(){
  ...
  glDrawArrays(GL_TRIANGLES,0,3+(counter%2)*3);
  glFlush();
}
